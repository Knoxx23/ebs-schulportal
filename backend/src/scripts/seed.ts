...
async function seed(): Promise<void> {
  ...
  const existingTeacher = db.prepare('SELECT id FROM users WHERE email = ?').get('teacher1@schule.de');
  if (existingTeacher) {
  ...
  const hashPassword = (pwd: string): string => bcryptjs.hashSync(pwd, 12);
  ...
  teacherIds.push((db.prepare('SELECT id FROM users WHERE email = ?').get('teacher1@schule.de') as any).id);

  db.prepare(`
    INSERT INTO users (email, password_hash, name, role, is_active)
    VALUES (?, ?, ?, ?, 1)
  `).run('teacher2@schule.de', teacherPwd, 'Benjamin Schmidt', 'teacher');
  teacherIds.push((db.prepare('SELECT id FROM users WHERE email = ?').get('teacher2@schule.de') as any).id);

  db.prepare(`
    INSERT INTO users (email, password_hash, name, role, is_active)
    VALUES (?, ?, ?, ?, 1)
  `).run('teacher3@schule.de', teacherPwd, 'Charlotte Fischer', 'teacher');
  teacherIds.push((db.prepare('SELECT id FROM users WHERE email = ?').get('teacher3@schule.de') as any).id);
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const inv = db.prepare('SELECT id FROM invitations WHERE token = ?').get(token) as any;
  ...
  for (let i = 0; i < 3; i++) {
    ...
    `).run(
      ...
      lastName,
      firstName,
      generateRandomDate(2018, 2019),
      `${lastName} ${firstName}`,
      `${getRandomElement(germanStreets)} 42`,
      `${80000 + Math.floor(Math.random() * 1000)}`,
      getRandomElement(germanCities),
      `0${Math.floor(Math.random() * 900000000000)}`,
      `${firstName.toLowerCase()}.${lastName.toLowerCase()}@beispiel.de`
    );
  }
  ...
  for (let i = 3; i < 7; i++) {
    ...
    `).run(
      ...
      generateRandomDate(2018, 2019),
      `${lastName} ${firstName}`,
      `${getRandomElement(germanStreets)} 23`,
  ...
  for (let i = 7; i < 9; i++) {
    ...
    `).run(
      ...
      generateRandomDate(2018, 2019),
      `${lastName} ${firstName}`,
      `${getRandomElement(germanStreets)} 15`,
  ...
  `).run(
    ...
    generateRandomDate(2018, 2019),
    `${lastName} ${firstName}`,
    `${getRandomElement(germanStreets)} 88`,
